/**
 * Title:Common.java
 * Author:riozenc
 * Datetime:2017年2月22日 下午3:38:18
**/
package sds.common.remote.util;

public class Common {

	public static final String CHARSET = "UTF-8";

	// // 公钥
	// public static final String PUBLICKKEY =
	// "";
	// // Key
	// public static final String KEY = "BD161A60C8933E7EC1D1B802376D6245";
	// // 请求URL
	// public static final String URL =
	// "http://mdx.branding.chinavalleytech.com/ChannelConn/Kubei";
	// // 实时注册接口
	// public static final String REGISTERURL =
	// "http://mdx.branding.chinavalleytech.com/ChannelConn/rlregister";

	// 公钥
	public static final String PUBLICKKEY = "";
	// Key
	public static final String KEY = "BD161A60C8933E7EC1D1B802376D6245";
	// 请求URL
	public static final String URL = "http://callback.chinavalleytech.com/hfcb/Kubei";
	// 实时注册接口
	public static final String REGISTERURL = "http://callback.chinavalleytech.com/hfcb/rlregister";

	public static final String CBZID = "820170111160659315592";

	public static final String CODE = "354036";
}
